...
/**
 * @FilePath     : /test-trt/include/private/trt/trt_common/trt_infer_schedule.hpp
 * @Description  :
 * @Author       : naonao
 * @Date         : 2025-01-19 10:26:18
 * @Version      : 0.0.1
 * @LastEditors  : naonao
 * @LastEditTime : 2025-01-20 14:19:39
...
public:
    struct Job{
        ...
        TRT::TimeCost preTime;
        TRT::TimeCost inferTime;
        TRT::TimeCost hostTime;
        TRT::TimeCost totalime;
    ...
    void stop(){
        ...
        for(int i =0 ;i<worker_vec_.size(); i++){
            worker_vec_[i]->join();
            worker_vec_[i].reset();
        }
        worker_vec_.clear();
    ...
    std::vector<bool> startup(const StartParam& param,int muti_context){
        run_ = true;
        start_param_ = param;
        std::vector<bool> results;
        for(int i = 0; i < muti_context; ++i){
            std::promise<bool> pro;
            worker_vec_.emplace_back(std::make_shared<std::thread>(&InferController::worker, this, std::ref(pro)));
            results.push_back(pro.get_future().get());
        }
        return results;
    }
...
protected:
    ...
    std::vector<std::shared_ptr<std::thread>> worker_vec_;
