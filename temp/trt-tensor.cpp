...
#include "../../../include/private/trt/trt_common/trt-tensor.hpp"
...
const char* Tensor::descriptor() const
{
    ...
    snprintf(descriptor_ptr, sizeof(descriptor_string_), "Tensor:%p, %s, %s, CUDA:%d", data_.get(), data_type_string(dtype_), shape_string_, device_id);
...
Tensor& Tensor::copy_from_gpu(size_t offset,  const void* src, size_t num_element, int device_id)
{
    ...
    if (head_ == DataHead::Device) {
        ...
        if (current_device_id != gpu_device_id) {
            checkCudaRuntime(cudaMemcpyPeerAsync(
                gpu<unsigned char>() + offset_location, gpu_device_id, src,
                current_device_id, copyed_bytes, stream_));
            // checkCudaRuntime(cudaMemcpyAsync(gpu<unsigned char>() +
            // offset_location, src, copyed_bytes, cudaMemcpyDeviceToDevice,
            // stream_));
        }
        else {
...
Tensor& Tensor::to_half()
{
    ...
    auto     c = count();
    float16* convert_memory =
        (float16*)malloc(c * data_type_size(DataType::Float16));
...
bool Tensor::set_bytes(size_t size)
{
    if (bytes_ <size) {
        bytes_ = size;
        return true;
