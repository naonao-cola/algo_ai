...
int test_anomalib()
{
    ...
    auto infer = anomalib::create_infer(modelPath, 0);
    ...
    for (auto item : rstlists) {
      /*  auto boxes  = item.get();
        ...
        cv::Mat heatMap = boxes[0].heatMap;
    }
    double total_cost   = time_cost.get_cost_time();
    double    per_cost_img = total_cost / mat_lists.size();
    LOG_INFO("===================summary=====================");
    LOG_INFO("total cost: {} s, cnt: {}, per_cost_img: {} s", total_cost/1000, mat_lists.size(), per_cost_img/1000);
