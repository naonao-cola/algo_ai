...
public:
    virtual ~InferImpl();
    virtual bool                                  load(const std::string& file, std::vector<std::vector<int>> dim={});
    ...
    virtual nvinfer1::IExecutionContext* get_context() override;
...
std::shared_ptr<std::vector<uint8_t>> InferImpl::serial_engine()
{
    ...
    auto output = make_shared<std::vector<uint8_t>>(
...
bool InferImpl::load(const std::string& file, std::vector<std::vector<int>> dim)
{
...
size_t InferImpl::get_device_memory_size()
{
    ...
    return context->context_->getEngine().getDeviceMemorySize() +
           inputs_[0]->bytes() + outputs_[0]->bytes();
}
...
void InferImpl::build_engine_input_and_outputs_mapper()
{
    ...
    if (dim_.size()>0){
        for (int i=0;i<dim_.size();i++){
            nvinfer1::Dims oriDims = context->context_->getBindingDimensions(i);
            for(int j=0;j<dim_[i].size();j++){
                if(dim_[i][j] != -1){
                    oriDims.d[j] = dim_[i][j];
                }
                context->context_->setBindingDimensions(i, oriDims);
                //context->context_->setBindingDimensions(i, nvinfer1::Dims4(dim_[i][0], dim_[i][1], dim_[i][2], dim_[i][3]));
            }
        }
    }
    ...
    for (int i = 0; i < nbBindings; ++i) {
        //获取运行时维度
        auto dims = context->context_->getBindingDimensions(i);
...
CUStream InferImpl::get_stream() { return this->context_->stream_; }
...
{
    return std::find(outputs_name_.begin(), outputs_name_.end(), name) !=
           outputs_name_.end();
}

bool InferImpl::is_input_name(const std::string& name)
{
    return std::find(inputs_name_.begin(), inputs_name_.end(), name) !=
           inputs_name_.end();
}

nvinfer1::IExecutionContext* InferImpl::get_context()
{
    EngineContext* context = (EngineContext*)context_.get();
    return context->context_.get();
}
void InferImpl::forward(bool sync)
{
    ...
    // bool execute_result = context->context_->enqueue(inputBatchSize,
    // bindingsptr, context->stream_, nullptr);
    bool execute_result = context->context_->enqueueV2(bindingsptr, context->stream_, nullptr);
    //bool execute_result = context->context_->executeV2(bindingsptr);
...
std::shared_ptr<MixMemory> InferImpl::get_workspace() { return workspace_; }

int InferImpl::num_input() { return static_cast<int>(this->inputs_.size()); }

int InferImpl::num_output() { return static_cast<int>(this->outputs_.size()); }
...
std::shared_ptr<Infer> load_infer(const string& file,std::vector<std::vector<int>> dim)
{
    // load tensorRT engine
    std::shared_ptr<InferImpl> Infer(new InferImpl());
    if (!Infer->load(file, dim)) {
        Infer.reset();
    }
    return Infer;
}
